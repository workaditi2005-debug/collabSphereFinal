...
def ai_generate():
    """Proxy endpoint for AI API calls (server-side) to avoid CORS and hide API key."""
    ...
    try:
        ...
        if not ANTHROPIC_API_KEY:
            return jsonify({'success': False, 'error': 'Server missing AI API key'}), 500

        # Forward the request to the Anthropics API
        url = 'https://api.anthropic.com/v1/messages'
        headers = {
            'Content-Type': 'application/json',
            # common pattern; if your provider expects Authorization, change accordingly
            'x-api-key': ANTHROPIC_API_KEY
        }

        resp = requests.post(url, headers=headers, json=data, timeout=30)

        # If the AI provider returned non-JSON, forward raw text
        try:
            payload = resp.json()
        except Exception:
            payload = {'text': resp.text}

        return (jsonify({'success': True, 'status_code': resp.status_code, 'data': payload}), resp.status_code)

    except requests.exceptions.RequestException as re:
        print(f"❌ AI proxy request failed: {re}")
        return jsonify({'success': False, 'error': 'AI request failed', 'details': str(re)}), 502
...
def send_collaboration_request():
    ...
    try:
        ...
        if not all([teammate_id, project_id, message]):
            return jsonify({
                'success': False, 
                'error': 'Missing required fields: teammateId, projectId, or message'
            }), 422
